...
async function loadLevel(dir) {
  ...
  try {
    const res = await fetch(`./entities/${dir}.ini`);
    ...
    statusEl.textContent = `Failed to load ${dir}: ${err.message} (serve viewer/ after running nicku-extract, e.g. python3 -m http.server -d viewer)`;
  }
}
